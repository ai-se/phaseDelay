...
the commonly held belief we label the {\em delayed issue effect} (hereafter, DIE):
...
This theory has been used as the basis for decision-making in software engineering. For example,  Basili and Boehm comment that, since the 1980s,  this effect
 \begin{quote}
...
\end{quote}
 \begin{figure}[!b]  
\begin{center}
\includegraphics[,width=4in]{img/b81.png}
 \end{center}
...
We examined the literature on the delayed issue effect through a combination of snowball sampling~\cite{wohlin2014guidelines} and database search. We began with \cite{boehm2012}, the most recent publication by Barry Boehm (at the time of this paper's first draft) that discusses the delayed issue effect. Boehm cites the seminal \textit{Software Engineering Economics}~\cite{Boehm81}.
%, which is bases the original delayed issue effect on four primary sources in the late 70s from IBM~\cite{Fagan76}, TRW~\cite{Boehm76}, GTE~\cite{Daly77}, and Bell Labs~\cite{Stephenson76}. 
We searched Google Scholar for terms such as ``cost to fix'' and ``defect cost'' and ``software quality cost''. The majority of the search results discuss quality measurements or quality improvement, but relatively few discuss cost-to-fix as a function of when the defect was injected or found. Our literature search is by no means exhaustive, but our search results yielded many popular sources such as~\cite{pressman2005software, boehm01,glass02,endres03}\footnote{These sources have a combined citation count of over 14,500 on Google Scholar}, all of which cite \textit{Software Engineering Economics}~\cite{Boehm81} or its derivative works.
...
%These seminal studies are the basis for most DIE claims and that the cost to find and fix a defect monotonically increases with lifecycle phase.  




 
In the 40 years since these initial studies, few studies have explored the cost to resolve issues
...
Some studies that report less-than large
 increase in the effort required to fix delayed issues. Royce~\cite{Royce98} studied  a million-line, safety-critical missile defense system. Design changes (including architecture changes) required approximately twice the effort of implementation and test changes, and the cost-to-fix in implementation and test phases increased slowly. Boehm~\cite{Boehm10} attributes this success to the  development process, which focused on removing architecture risk early in the development lifecycle. Boehm~\cite{Boehm80} provides data suggesting that the cost-to-fix curve for small projects, two student projects of 2000 deliverable source instructions, is flatter than for large projects (the dashed line of Figure~\ref{fig:cost-to-fix}). Data from NASA's Johnson Space Flight Center, reported by Shull~\cite{Shull02}, found that the cost to fix certain non-critical classes of defects was fairly constant across lifecycle phases (1.2 hours on average early in the project, versus 1.5 hours late in the project). Again, the data points from these studies are not available, and thus newer evidence both in favor of and contrary to the DIE cannot be evaluated. 
...
most treatments achieved the same ranks i.e. they were found to be insignificantly different  (the one exception is the Planning:UnitTest results 
...
The twp key features of \fig{raw} are:
\be
\item Nowhere in
...
We do not claim that this effect {\em never} holds in software projects; just that it cannot be assumed to {\em always} hold. Our explanation of the observed lack-of-effect is five-fold. Each of the following explanations is essentially a hypothesis which should be tested against empirical data before we can effectively propose a new theory of the delayed issue effect.
\be
