...
\sei{
\bi
    \item What are artifacts uploaded to the SEI tool?
    \item What are the units contained in those artifacts? Are they the ``issues'' as defined in the following paragraph?
    \item Is participation in the SEI data collection mandatory? i.e., is the data provided self selected?
    \item When is the data collected?
    \item What are screening/cleaning procedures, if any?
\ei
}

\sei{ wrn: please review the following}
...
\bi 
\item
Several  phases have the similar  sub-phases of {\em review} and {\em inspect}. In TSP, individuals perform personal reviews on products prior to the peer inspection.\footnote{\bill{plz distinguish review and inspect. does one usually happen first?}}
\item Testing is divided into several statges \footnote{\bill{
(this appears to be OBE), need a one
