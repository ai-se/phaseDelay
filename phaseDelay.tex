...
One technical aside: there are several ways to  measure {\em difficulty}. The first is a direct measure of the actual work time spent in correcting the defect in the product. Direct measurement enables analysis of the variability the cost-to-fix among a category of defects. The second approach measures the total time spent in a defect removal phase and divides by the total defects removed. This provides an average cost-to-fix including overhead of that activity. For example, a code inspection may require an hour of effort to remove three defects for a 3 defects per hour or to invert the ratio, 1/3 hours per defect. One observation made by Nichols while studying our 171 projects is that most of the time  is spent locating the defects not actually fixing them. we use the second approach in our analysis.
...
deployment} might have been used to predict for exponentially increasing
difficulty {\em post-deployment}.
For example:
\bi
...
of an exponential delay effect in the difficulty to resolve requirements issues during development.
The greatest difficulty in resolving requirements issues was 1.31 seen in systems testing--
which is far smaller than the 37 to 250-fold increases reported in \fig{b81}, \fig{cost-to-fix},
and \fig{steck}.  That is, from our TSP data, we would not anticipate exponentially greater difficulty in resolving requirements issues after
deployment. 
...
To sum up the paper so far,
\bi
\item
The delayed issue effect has been reported in around a dozen papers, at least half of which date back to the 1970s. 
\item
But there exists
no evidence for this effect was seen  in hundreds of software projects build   since 2006.
\ei
From this second point, is it valid to assert that it is {\em not} disruptive to  software projects
to let some issues advance from one phase to the next?

\subsection{Alternate Explanations}

One explanation for the above is that our sample is biased-- that while general software
...
It would be convenient to offer the last paragraph as the conclusion of this study. Two of our authors (Nichols and Shull)
 work at the organization that invented and promoted TSP.



\subsubsection*{H4: A post-development DIE effect can be predicted from pre-deployment data} 
The data collected from our 171 projects     pertains only to
development, and not to subsequent operations.
So did we miss the DIE effect in our data since
our data collection is just not prolonged enough to detect that effect?

We would argue that this is not the case.
An  exponential DIE effect can be seen {\em during the development} of all
the papers we have reviewed. For example:
\bi
\item In \fig{b81}: see the data relating to development and acceptance testing;
\item In  all the case studies of \fig{steck} in the columns {\em Requirements} to {\em Test}.
\ei
That is, in the literature we have surveyed, the post-development DIE effect would have been predicted by data collected during pre-deployment development.
Such 

Another surprise was a lack of {\em growth trend} suggesting that, in operation,
 we should expect larger exponential delays in resolving issues. This is odd since all
 the papers we could access on DIE should such a growth trend in the pre-operational data.
  



\subsubsection*{H5: ``Time'' to resolve an issue is analogous to ``cost''}
This hypothesis a somewhat technical point needed for our argument.
While classical papers on the delayed issue effect discuss the {\em cost}
of delaying issues, the data from our 171 projects only has time data.
Hence, to say that our data addresses the classic delayed issue effect,
we need to make the case that our time measurements are an appropriate
surrogate for the prior claims on the cost of delaying issues.
...
XXXX die does not aoccur during developmentjust during deployment

that means during period requirements to test pre-deployent, there is not rush to resolve issues (just as long as a concerted effort is made to retire all issues priot to deployment)

all our data is skewed due to TSP. two responses. 1) we don't thing so. 2) sure, let it due to TSP in which case we can fix the delayed issue effect via process methods 9everyone adopts the team software process). In either case, the delayed issue effect can be ignored or mitigated without extensive investment in radically new methods or tools (e.g. automatic formal methods, better requirement description languages).
...
Consider the 23,000,000 veterans working with  the U.S.  Veterans 
