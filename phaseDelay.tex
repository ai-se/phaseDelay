...
For example, consider the MEAN stack preferred for web development by
...
use Javscript throughout-- which makes large scale reorganizations faster to complete~\cite{wayner15}.
For another example where organizations are adopting increased development flexability, 
...
Also,
upgrades to their Windows operating system is moving from service patches (which occur rarely) to continuous deployment (so there will be no Windows 11- just a stream of continuous updates to what is currently called Windows 10~\cite{bright15}).

Other large development shops are similarly reorganizing their work-- witness the US Department of Defense's 2010 mandate
...
Hence, the rest of this article explores the currency of the delayed issue effect.
After some initial definitions for terminology and hypotheses, we discuss the
value of checking old ideas. Next, we offer a  survey of industrial practitioners and researchers to show that this the negative impact of issue delay is the very prominent
concern in thos communities (in fact, amongst industrial practitioners, it is their number one concern).   Next, we  analyze 171 software  projects developed in the period 2006 to 2014 to find  {\em no evidence} of the delayed issue effect
...
\subsubsection*{H1: DIE is a current and widely-held belief}
...
\subsubsection*{H2: DIE is a poorly documented effect}
 
As discussed in our literature review,  many of the papers reporting the DIE
effect are either:
\bi
\item
Quite old (dating from last century);
\item
Just quote prior papers without presenting   new data; 
\item
Or cite data sources that can no longer be
confirmed.
\ei

\subsubsection*{H3: During development, issues   exhibit a DIE effect}
While this paper  endorse the above hypotheses, we will argue against this
third hypothesis. In our sample of 
 171 software  projects developed, we will show
{\em no evidence}  that, during development, issues were exponentially harder to resolve the longer they were left
 in the software. 
...
Before going any furhter,  we digress to discuss the merits of revisiting
...
\section{H1: DIE is a current and widely-held belief}
...
\section{H2: DIE is a poorly documented effect}
...
\item In  all the case studies of \fig{steck04} in the columns {\em Requirements} to {\em Test}.
\ei
...
The authors gratefully acknowledge the careful comments of anonymous reviewers-- particularly, their comments on issues relating to ``cost''
and ``time''.
