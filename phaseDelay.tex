...
\ei
 ...
 \fig{phase_fixtime_plot} shows scatterplots of the cost-to-fix (rank) per phase against the phase delay. In the early phases, i.e., Design \& Coding, most of the defects found have a relatively small phase delay. This is ideal considering that the purpose of the early phase reviews is to remove as many defects as possible. 

In the TSP data, there is a weak correlation (Kendall's $\tau = 0.19, p < 0.01, 95\% CI$) between a defect's cost-to-fix (minutes of effort) and the defects phase delay (phase removed - phase injected) - see \fig{correlation}. The correlations were also calculated for each phase, e.g., does the phase delay of defects found in Test correlate with the defect's cost-to-fix (\fig{phase_corr}).  
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=3.3in]{corr.png}
\end{center}
...
\begin{tabular}{lll}
  ...
  \hline
  Design & 0.14 & 0.08 \\ 
  DesignReview & 0.06 & $<$0.01 \\ 
  DesignInspect & 0.05 & $<$0.01 \\ 
  Code & 0.27 & $<$0.01 \\ 
  CodeReview & 0.12 & $<$0.01 \\ 
  CodeInspect & 0.09 & $<$0.01 \\ 
  Test & 0.13 & $<$0.01 \\ 
  IntTest & 0.1 & $<$0.01 \\ 
  SysTest & 0.25 & $<$0.01 \\ 
  AcceptTest & 0.06 & 0.29 \\ 
  \end{tabular}
\caption{Correlation of cost-to-fix and phase delay for defects found (Kendall's $\tau$)} 
\label{fig:phase_corr}
\end{figure}

\begin{figure*}[!ht]
\centering
\includegraphics[width=1.6in]{corr-Code.png}
\includegraphics[width=1.6in]{corr-CodeReview.png}
\includegraphics[width=1.6in]{corr-CodeInspect.png}
\vspace{0.01cm}

\includegraphics[width=1.6in]{corr-Design.png}
\includegraphics[width=1.6in]{corr-DesignReview.png}
\includegraphics[width=1.6in]{corr-DesignInspect.png}
\vspace{0.01cm}

\includegraphics[width=1.6in]{corr-Test.png}
\includegraphics[width=1.6in]{corr-IntTest.png}
\includegraphics[width=1.6in]{corr-SysTest.png}
\includegraphics[width=1.6in]{corr-AcceptTest.png}

\caption{Correlation of cost-to-fix (rank) and phase delay for defects found by phase for phases where \# defects >= 30 and \# unique phase delay values >= 5. x = phase delay, y = cost-to-fix rank. Plots use random samples from each phase for readability.}
\label{fig:phase_fixtime_plot}
\end{figure*}
